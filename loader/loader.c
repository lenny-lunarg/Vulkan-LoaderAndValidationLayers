    ...
    if (out_files->filename_list != NULL) {
        for (uint32_t i = 0; i < out_files->count; i++) {
            if (NULL != out_files->filename_list[i]) {
                // TODO: Add this back in. Removing it will cause a memory leak, but adding it back in will cause a crash.
                // Most likely, this is causing a double free that needs to be tracked down in more detail.
